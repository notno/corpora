...
def build_user_prompt(term: str, context: str = "") -> str:
    ...
    """
    if context:
        return f"Classify this fantasy term: '{term}'\n\nContext: {context}"
    return f"Classify this fantasy term: '{term}'"
...
class BatchClassifier:
    ...
    def create_batch(
        self,
        terms: List[tuple[str, str]],  # (term, source) pairs
    ) -> str:
        """Create a batch request for term classification.

        Args:
            terms: List of (term, source) tuples to classify

        Returns:
            Batch ID for tracking
        """
        requests = []
        for i, (term, source) in enumerate(terms):
            requests.append(
                Request(
                    ...
                    params=MessageCreateParamsNonStreaming(
                        ...
                        messages=[
                            {"role": "user", "content": build_user_prompt(term)}
                        ],
                    ),
                )
            )
...
class TestBatchClassifier:
    ...
    def test_create_batch(self, mock_anthropic):
        ...
        batch_id = classifier.create_batch([("fireball", "test"), ("dragon", "test")])
